...
class Factories:
    ...
    def generateStudentSubmissionsForAssignment(_studentRoster: list[dict], _assignmentID: str) -> list[dict]:
        ...
        Grade distribution will be assigned on a normal curve, similar to
        ``Factories.generateStudentGradescopeGradesForAssignment``
        ...
        if Factories.AssignmentMaxStudentPoints is not None:
            # On average students get an 75% - 80% with a standard deviation of 12% - 15%. Using a 75% avg and a 15%
            #  sd for this
    ...
    def generateSingleStudentGradescopeGrade(_studentRosterEntry: dict, _studentScore: float | None,
        ...
        returns in format
        ``{First Name},{Last Name},{Student ID},{Student Email},{Student Score},{Max Score},{Status},{Lateness}``

        :param _studentRosterEntry: The student to create an entry for. Should be generated by
        ``Factories.generateStudentRoster`` :param _studentScore: The score the student received on their submission.
        :param _assignmentPoints: The max number of points for this assignment. :param _status: The status for the
        student's submission. Must be ``Graded``, ``Missing``, or ``Ungraded``. :param _lateness: How late the
        students submission was. Stored in hours. If ``None`` is set then ``_status`` must be ``Missing``
    ...
    @staticmethod
    def generateSpecialCases(_studentRoster: list[dict], _assignmentCommonName: list[str],
                             extensionTypes: list[str] | None = None,
                             specialCaseRatio: float = 0.05):
        """
        :Description:

        This function generates a special cases spreadsheet for the students found in the roster. By default,
        it uses a frequency of 5% meaning that 5% of students will have a special case for that assignment.

        Writes out special cases spreadsheet to ``special_cases/SC_test_working.xlsx``

        :param _studentRoster: the roaster for the students as generated by ``Factories.generateStudentRoster``
        :param _assignmentCommonName: the list of the assignment common names to generate special cases for
        :param extensionTypes: the list of possible extensions.
        ...
        hasExtension: list[str] = ["Extension", "NoExtension"]
        hasExtensionWeights: list[float] = [specialCaseRatio, 1 - specialCaseRatio]

        for assignmentCommonName in _assignmentCommonName:
            for student in _studentRoster:
                if random.choices(population=hasExtension, weights=hasExtensionWeights, k=1)[0] != "Extension":
                    continue

                newSpecialCase: pd.DataFrame = pd.DataFrame(
                    {
        ...
        filename: str = "./special_cases/SC_test_working.xlsx"
